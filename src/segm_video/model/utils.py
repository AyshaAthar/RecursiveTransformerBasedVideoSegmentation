...
def checkpoint_filter_fn(state_dict, model):
    ...
    for k, v in state_dict.items():
        if k == "vit_embs.encoder.pos_embed" and v.shape != model.pos_embed.shape:
...
def sliding_window(im, flip, window_size, window_stride):
    """ Divide image sinto windoes of fixed size """
    B, T, C, H, W = im.shape
    ws = window_size
...
def merge_windows(windows, window_size, ori_shape):
    """ Merge windows that were generated to riginal image size """
    ws = window_size
    ...
    H, W = windows["shape"]
    flip = windows["flip"]
    T=im_windows[0].shape[0]

    logit = torch.zeros((T,C, H, W), device=im_windows.device)
    count = torch.zeros((T,1, H, W), device=im_windows.device)
    ...
    logits=[]
    for i in range(T):
        logits.append(F.interpolate(
        logit[i].unsqueeze(0),
        ori_shape,
        mode="bilinear",
    )[0])
    finallogit = torch.stack(logits)
...
def merge_windows_vit(windows, window_size, ori_shape):
    ws = window_size
    im_windows = windows["vit_maps"]
    anchors = windows["anchors"]
    C = im_windows[0].shape[1]
    H, W = windows["shape"]
    flip = windows["flip"]
...
def inference(
    ...
    """
    Evaluation function, uses sliding window approach
    ...
    model: 
        Initialised model
    ims: Tensor
        Images used, size : B * T * C * H * W
    ims_metas: Tensor
        Contains information about the sequence being processed.
    ori_shape: tupe
        Original shape of image
    window_size: int
        Window size used for sliding window     
    window_stride: int
        Strude used for sliding iwndow
    batch_size: int
        Batch size used
    """
    
    B1, T, C, H, W = ims.shape
    ori_h, ori_w = ori_shape
    C = model.n_cls
    seg_map = torch.zeros((T, C, ori_h, ori_w), device=ptu.device)
    vit_map = torch.zeros((T, C, ori_h, ori_w), device=ptu.device)
    ori_shape = [ori_h, ori_w]
    for im, im_metas in zip(ims, ims_metas):
        im = im.to(ptu.device)
        im = resize(im, window_size)
        im = im.reshape(1,T,3,ori_h, ori_w)
        flip = False
        windows = sliding_window(im, flip, window_size, window_stride)
        crops = torch.stack(windows.pop("crop"))[:, 0]
        B = len(crops)
        WB = batch_size
        seg_maps = torch.zeros((B, T, C, window_size, window_size), device=im.device)
        vit_maps = torch.zeros((B, T, C, window_size, window_size), device=im.device)

        with torch.no_grad():
            for i in range(0, B, WB):
                seg_maps[i : i + WB], vit_ims, pred_ims, corr_ims = model.forward(crops[i : i + WB])
                #seg_maps[i : i + WB], vit_ims, pred_ims, corr_ims,vit_maps[i : i + WB]  = model.forward(crops[i : i + WB])
        windows["seg_maps"] = seg_maps
        windows["vit_maps"] = vit_maps
        
        im_seg_map = merge_windows(windows, window_size, ori_shape)
        vit_seg_map = merge_windows_vit(windows, window_size, ori_shape)
        
        seg_map += im_seg_map
        vit_map += vit_seg_map
    seg_map /= len(ims)
    seg_map = seg_map.reshape(B1,T,C,H,W)
    return seg_map,pred_ims,corr_ims,vit_ims
    #return seg_map,pred_ims, corr_ims,vit_ims,vit_map

def num_params(model):
    """ Function to find number of trainable params """
